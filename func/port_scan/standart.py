...
write_log("[&] 'standart.py' dosyası çalıştırıldı", level="EXEC")
...
def standart(hedef_ip=None, timeout=0.5, max_workers=None, scanner_func=None, os_discovery=False, verbose=False):
          write_log("[~] 'standart()' fonksiyonu çalıştırıldı", level="FUNC")
          if not hedef_ip:
          ...
          port_listesi = list(range(1, 1025))
          ...
          print(f"\n{clr.am}{get_string('total_scanned')}:{clr.r} 1024 {clr.am}{get_string('open_port_count')}:{clr.r} {len(acık_liste)}\n{clr.am}{get_string('target_ip')}:{clr.r} {hedef_ip} {clr.am}{get_string('duration')}:{clr.r} {tson_sure}")
          write_log(f"[#] {get_string('total_scanned')}: 1024 | {get_string('open_port_count')}: {len(acık_liste)} | {get_string('target_ip')}: {hedef_ip} | {get_string('duration')}: {tson_sure}", level="RESULT")
