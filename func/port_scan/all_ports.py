...
write_log("[#] 'all_ports.py' dosyası çalıştırılı")
...
def tum_portlar():
          write_log("[#] 'tum_portlar()' fonksiyonu çalıştırıldı")
          ...
          write_log(f"[#] Toplam taranan: 65535 | Açık port sayısı: {len(acık_port)} | Hedef IP:{hedef_ip}")
          if not acık_port:
